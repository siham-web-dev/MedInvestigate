...
import {
  TabNavigation,
  type TabId,
} from "../components/workspace/TabNavigation";
import { MsgBubble } from "../components/workspace/MsgBubble";
import {
  PanelSection,
  Detail,
  FindingItem,
} from "../components/workspace/PanelSection";
import {
  GraphTab,
  ReviewTab,
  ReportTab,
...
const parseFindingsFromActivities = (
  ...
  activities.forEach((activity) => {
    ...
    if (
      (activity.agentType === "Regulatory" &&
        activity.message.includes("MDR")) ||
    ...
    if (
      activity.agentType === "Clinical" &&
      (message.includes("adverse") || message.includes("clinical"))
    ) {
    ...
    if (
      activity.agentType === "Technical" &&
      (message.includes("firmware") || message.includes("technical"))
    ) {
  ...
  return {
    ...findings,
    regulatory: Array.from(
      new Map(findings.regulatory.map((f) => [f.text, f])).values(),
    ),
    clinical: Array.from(
      new Map(findings.clinical.map((f) => [f.text, f])).values(),
    ),
    technical: Array.from(
      new Map(findings.technical.map((f) => [f.text, f])).values(),
    ),
    risk: findings.risk,
  };
};
      ...
      if (invResponse.ok) {
        ...
        if (invData.incidentId) {
          const incResponse = await fetch(
            API_ENDPOINTS.incident(invData.incidentId),
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
        ...
        if (invData.phase !== "Intake" && invData.phase !== "Analysis") {
          console.log(
            "[WORKSPACE] Investigation not in progress, fetching logs from database",
          );
          const logsResponse = await fetch(
            API_ENDPOINTS.investigationAgentLogs(id!),
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          ...
          if (logsResponse.ok) {
            ...
            console.log(
              "[WORKSPACE] Agent logs loaded from database:",
              logs.length,
            );
            ...
            const messages = logs.map((log: any) => ({
              ...
              message: log.message,
              severity:
                log.status === "Alert" ? ("alert" as const) : ("info" as const),
            }));
  ...
  useInvestigationSocket(
    ...
    (update) => {
      if (update.phase === "Analysis") {
        // Workflow is re-running - reset graph nodes to initial state
        setStreaming(true);
        // Clear all previous activities so graph nodes reset to idle/queued
        setVisibleMsgs([]);
      } else if (update.status === "done" || update.phase === "Complete") {
        setStreaming(false);
      }
    },
  );
  ...
  return (
    <div className="flex flex-col h-full">
      <InvestigationHeader
        ...
        device={
          (incident as any)?.deviceName ||
          (incident as any)?.device ||
          "Unknown Device"
        }
