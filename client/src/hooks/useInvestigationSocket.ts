...
export const useInvestigationSocket = (
  ...
  const [investigationStatus, setInvestigationStatus] = useState<string>('unknown');
  ...
  useEffect(() => {
    ...
    const fetchInvestigationStatus = async () => {
      try {
        const response = await fetch(API_ENDPOINTS.investigation(investigationId), {
          headers: {
  ...
  useEffect(() => {
    ...
    const shouldConnect = investigationStatus === 'Analysis' || investigationStatus === 'Intake';

    console.log(`[SOCKET] Status check - investigationStatus: ${investigationStatus}, shouldConnect: ${shouldConnect}`);

    if (!shouldConnect) {
      console.log('[SOCKET] Skipping socket connection - investigation not in progress');
      return;
    }
    ...
    socket.on('workflow-update', (update: WorkflowUpdate) => {
      console.log('[SOCKET] Workflow update received:', update.phase, update.status);

      // If investigation is complete or in review, disconnect
      if (update.phase === 'Complete' || update.phase === 'Review' || update.status === 'done') {
